...
(define (do-log-sequential-merge directory)
  (define files (filter (λ (s) (string-suffix? s CLOSED-FILE-EXTENSION))
                        (directory-list "test")))
  (define lsm-file (read-lsm-file directory))
  
  (define result
    (for/fold ([keyhash (hasheq)])
              ([file files])
      (define mrv (most-recent-values keyhash file))
      (print mrv)))
  (write-key-hash! result)
  result)

; String -> [Hash String FilePointer]
; produces the hash from all the data in the given directory to where to find their values
...
(module+ test
  ...
  (check-equal? (send bc get "a") "asdfasdf")

  (send bc put! 'a-number 130)
  (check-equal? (send bc get 'a-number) 130)

  
  (send bc put! 'g (hasheq 'blah 2))

  (check-equal? (send bc get 'g) (hasheq 'blah 2))
  ...
  (send bc put! 'g (hasheq 'blah 2))
  (check-equal? (send bc fold (λ (k v acc) (add1 acc)) 0) 103)
  )
